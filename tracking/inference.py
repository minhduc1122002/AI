...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.allPositions:
            probability = self.getObservationProb(observation, gameState.getPacmanPosition(), ghostPos, self.getJailPosition())
            # B(X t+1) = P(e t+1 | X t+1) * B(X t)
            self.beliefs[ghostPos] *= probability
        self.beliefs.normalize()
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                uniProb *= self.getObservationProb(observation[i], pacmanPosition, particle[i], jailPosition)
            weightDist[particle] += uniProb
